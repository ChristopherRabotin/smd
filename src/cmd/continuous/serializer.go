...
func CGOut(name, center string, startDT, endDT time.Time) {
	...
	item := dataio.CgItems{Class: "spacecraft", Name: name, StartTime: fmt.Sprintf("%s", startDT.UTC()), EndTime: fmt.Sprintf("%s", endDT.UTC()), Center: center, Trajectory: &traj, Bodyframe: nil, Geometry: nil, Label: &label, TrajectoryPlot: &plot}
