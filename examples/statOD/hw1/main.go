...
func main() {
	...
	mis := smd.NewMission(smd.NewEmptySC("hw1", 0), osc, dt, dt.Add(24*time.Hour), smd.Cartesian, pert, smd.ExportConfig{Filename: "hw1.0", Cosmo: true, AsCSV: true, Timestamp: false})
	mis.Propagate()

	// Second with arbitrary perturbation.
	pert.Arbitrary = func(o smd.Orbit, m smd.Propagator) []float64 {
		return []float64{1, 0, 0, 0, 10e-3, 0, 0}
	}
	osc = smd.NewOrbitFromOE(7000, 0.001, 30, 80, 40, 0, smd.Earth)
	mis = smd.NewMission(smd.NewEmptySC("hw1", 0), osc, dt, dt.Add(24*time.Hour), smd.Cartesian, pert, smd.ExportConfig{Filename: "hw1.1", Cosmo: true, AsCSV: true, Timestamp: false})
	mis.Propagate()
}
