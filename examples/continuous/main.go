...
func main() {
	...
	*/
	...
	scMars.LogInfo()
	astroM := smd.NewMission(scMars, InitialMarsOrbit(), end, endM, false, smd.ExportConfig{Filename: "IM", AsCSV: false, Cosmo: false, Timestamp: false})
	astroM.Propagate()
	...
	sc.LogInfo()
	astro := smd.NewMission(sc, InitialEarthOrbit(), start, end, false, smd.ExportConfig{Filename: "IE", AsCSV: true, Cosmo: true, Timestamp: false})
	astro.Propagate()
