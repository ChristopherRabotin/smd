...
func TestMissionStop(t *testing.T) {
	...
	for _, meth := range []Propagator{Cartesian, GaussianVOP} {
		...
		sc.FuelMass = -1
		astro := NewMission(sc, o, start, end, meth, Perturbations{}, ExportConfig{})
		// Start propagation.
		go astro.Propagate()
...
func TestMissionFrameChg(t *testing.T) {
	...
	astro := NewMission(NewEmptySC("test", 1500), o, start, end, GaussianVOP, Perturbations{}, ExportConfig{})
	// Start propagation.
	astro.Propagate()
...
func TestMultiCorrectOE(t *testing.T) {
	for _, prop := range []Propagator{GaussianVOP, Cartesian} {
		for _, meth := range []ControlLawType{Ruggerio, Naasz} {
			...
			astro := NewMission(sc, oInit, start, end, prop, Perturbations{}, ExportConfig{Filename: fmt.Sprintf("ruggMulti-%s-%s", prop, meth), Cosmo: true, AsCSV: true})
			astro.Propagate()
...
func TestPetropoulosCaseA(t *testing.T) {
	...
	for _, prop := range []Propagator{GaussianVOP, Cartesian} {
		for _, meth := range []ControlLawType{Naasz} {
			...
			astro := NewMission(sc, oInit, start, end, prop, Perturbations{}, ExportConfig{Filename: fmt.Sprintf("petroA-%s-%s", prop, meth), Cosmo: false, AsCSV: true})
			astro.Propagate()
...
func TestPetropoulosCaseB(t *testing.T) {
	for _, prop := range []Propagator{GaussianVOP, Cartesian} {
		for _, meth := range []ControlLawType{Ruggerio, Naasz} {
			...
			astro := NewMission(sc, oInit, start, end, prop, Perturbations{}, ExportConfig{Filename: fmt.Sprintf("petroB-%s-%s", prop, meth), Cosmo: true, AsCSV: true})
			astro.Propagate()
...
func TestPetropoulosCaseC(t *testing.T) {
	for _, prop := range []Propagator{GaussianVOP, Cartesian} {
		for _, meth := range []ControlLawType{Ruggerio, Naasz} {
			...
			astro := NewMission(sc, oInit, start, end, prop, Perturbations{}, ExportConfig{Filename: fmt.Sprintf("petroC-%s-%s", prop, meth), Cosmo: false, AsCSV: true})
			astro.Propagate()
			aOsc, eOsc, _, _, _, _, _, _, _ := astro.Orbit.Elements()
...
func TestPetropoulosCaseE(t *testing.T) {
	...
	for _, prop := range []Propagator{GaussianVOP, Cartesian} {
		for _, meth := range []ControlLawType{Ruggerio, Naasz} {
			...
			astro := NewMission(sc, oInit, start, end, prop, Perturbations{}, ExportConfig{Filename: fmt.Sprintf("petroE-%s-%s", prop, meth), Cosmo: false, AsCSV: true})
			astro.Propagate()
