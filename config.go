...
func (c _smdconfig) HelioState(planet string, epoch time.Time) planetstate {
	...
	if conf.meeus {
		...
		w := valP - valW
		M := valL - valP
		Ccen := (2*e-math.Pow(e, 0.75)+5./96*math.Pow(e, 5))*math.Sin(M) + (5./4*math.Pow(e, 2)-11./24*math.Pow(e, 4))*math.Sin(2*M) + (13./12*math.Pow(e, 3)-43./64*math.Pow(e, 5))*math.Sin(3*M) + 103./96*math.Pow(e, 4)*math.Sin(4*M) + 1097./960*math.Pow(e, 5)*math.Sin(5*M)
		nu := M + Ccen
